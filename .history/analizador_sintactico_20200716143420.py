...
def p_metodo(p):
    '''metodo : ID PUNTO ID APAR CPAR
            ...
            | ID PUNTO FUNCIONES APAR CPAR
            | ID PUNTP FUNCIONES APAR ID CPAR
    '''
def p_atributo(p):
...
def p_condicion(p):
    '''condicion : ID comparador ID
                | ID comparador ID conector ID comparador ID
    ''' 
...
def p_control(p):
    '''control : if
                ...
                | while
    '''
#poner multiplicidad a sentencia
def p_cuerpo(p):
    '''cuerpo : sentencia
